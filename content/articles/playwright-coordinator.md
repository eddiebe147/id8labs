...
**How we created a multi-agent Playwright coordination system in one session—because that's what we do here.**
...
The Playwright Agent Coordination System is available in StackShack under **Code & Technical**.

**Installation:** Copy the `~/.claude/playwright-coordinator/` directory and add the hook configurations to your `settings.local.json`.
