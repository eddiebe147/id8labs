...
## Stealing a Design Language (With Permission From the Source Code)

I'd been admiring factory.ai's design for a while — the AI company that builds autonomous coding agents. Their site has this quality I couldn't name at first. Professional without being corporate. Dark without being gloomy. Technical without being cold.

So I did what any builder would do. I opened it in Playwright, extracted every CSS variable, and reverse-engineered their design system.
...
- **Warm neutral grays** — every gray has a brownish undertone, not the cool blue-gray that most dark themes use. This is what gives it warmth.
- **Geist font** — weight 400 (light!) for headings with tight letter-spacing (`-2px`). The signature move. Big type at thin weight reads elegant, not heavy.
- **No shadows. No gradients. No glow.** — The layout and typography *are* the design. Whitespace does the heavy lifting.
- **Orange accent** — `#ef6f2e`. One color. Used surgically: section indicator dots, active tabs, badges. Never splashed around.

I adapted about 90% of it. Kept their warm neutrals, their typography philosophy, their "less is more" component patterns. Added our own orange/amber accent palette and teal for success states. Called it "Factory-Inspired" and baked it into the `/visualize` command spec.

Every HTML visualization I generate now automatically follows this language. The command file has the complete token system — every CSS variable, every component pattern, every layout rule — so any future session produces consistent output without me explaining the design again.
...
*Credit to Shah for the FaceTime conversation that started all this, and to the factory.ai team for the design language that made it beautiful.*
