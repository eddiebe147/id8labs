...
// Helper to get context message based on redirect
function getContextMessage(redirect: string | null): { title: string; subtitle: string } | null {
  if (!redirect) return null

  // Check if redirecting from a paid course module
  if (redirect.startsWith('/courses/claude-for-knowledge-workers/module-')) {
    const moduleMatch = redirect.match(/module-(\d+)/)
...
function SignInForm() {
  ...
  return (
    <div className="bg-white dark:bg-bg-dark border border-border-light dark:border-border-dark rounded-lg p-8 shadow-lg">
      {/* Context-aware banner for course redirects */}
      {contextMessage && (
      ...
      <h1 className="text-3xl font-bold text-text-light dark:text-text-dark mb-2">
        {contextMessage ? 'Sign in to continue' : 'Welcome back'}
      </h1>
      <p className="text-text-light-secondary dark:text-text-dark-secondary mb-8">
        {contextMessage ? 'Your progress will be saved automatically.' : 'Sign in to continue your learning journey'}
      </p>
