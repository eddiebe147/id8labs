              ...
              <div className="relative h-64 md:h-80 rounded-xl border border-blue-500/20 bg-blue-500/5 overflow-hidden p-4">
                ...
                <div className="relative z-10 h-full flex flex-col gap-3">
                  ...
                  <div className="flex-1 space-y-2 overflow-hidden">
                    {[60, 75, 85, 70, 80].map((width, i) => (
                      <div key={i} className="flex items-center gap-2 p-2 rounded bg-white/5">
                        <div className="w-2 h-2 rounded-full bg-emerald-400" />
                        <div className="h-2 rounded bg-white/20" style={{ width: `${width}%` }} />
                        <div className="ml-auto h-2 w-12 bg-blue-400/40 rounded" />
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex-1 grid grid-cols-5 gap-1">
                    {[...Array(20)].map((_, i) => (
                      <div key={i} className="relative bg-white/5 rounded p-1">
                        {i === 7 && (
                          <>
                            <div className="absolute inset-0.5 bg-emerald-500/30 rounded border border-emerald-500/50" />
                            <div className="h-1 w-full bg-emerald-400/60 rounded mt-1" />
                          </>
                        )}
                        {i === 12 && (
                          <>
                            <div className="absolute inset-0.5 bg-blue-500/30 rounded border border-blue-500/50" />
                            <div className="h-1 w-full bg-blue-400/60 rounded mt-1" />
                          </>
                        )}
                        {i === 14 && (
                          <>
              ...
              <div className="relative h-64 md:h-80 rounded-xl border border-amber-500/20 bg-amber-500/5 overflow-hidden p-4">
                ...
                <div className="relative z-10 h-full flex flex-col gap-3">
                  ...
                  <div className="flex-1 space-y-2">
                    {[
                      { label: 'Contract Signed', status: 'complete' },
                      { label: 'Inspection', status: 'complete' },
                      { label: 'Appraisal', status: 'active' },
                      ...
                      <div key={i} className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full ${
                          ...
                          item.status === 'active' ? 'bg-amber-400 animate-pulse' :
                          'bg-zinc-600'
                        }`} />
                        <div className="flex-1 h-1.5 bg-white/10 rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${
                            item.status === 'complete' ? 'w-full bg-emerald-400/50' :
                            item.status === 'active' ? 'w-1/2 bg-amber-400/50' :
                            'w-0'
                          }`} />
                        </div>
                        <span className={`text-[9px] w-20 text-right ${
                          item.status === 'active' ? 'text-amber-400' : 'text-zinc-500'
                        }`}>{item.label}</span>
                      </div>
                    ))}
                  </div>
