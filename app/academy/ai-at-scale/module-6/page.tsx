      ...
      <section className="relative py-20 bg-zone-text">
        <div className="container">
          <m.div initial="initial" animate="animate" variants={stagger} className="max-w-3xl">
            ...
            <m.h1 variants={fadeUp} className="text-4xl md:text-5xl font-bold tracking-tight mb-6">Measuring Team ROI</m.h1>
            <m.p variants={fadeUp} className="text-xl text-[var(--text-secondary)]">Beyond individual time savings. Measuring aggregate team impact and organizational value.</m.p>
          </m.div>
        </div>
      </section>
      <section className="section-spacing">
        <div className="container"><div className="max-w-3xl mx-auto"><div className="p-8 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl text-center"><p className="text-lg text-[var(--text-secondary)] mb-4">Full module content coming soon.</p><p className="text-sm text-[var(--text-tertiary)]">You'll build: Team ROI dashboard design</p></div></div></div>
      </section>
      <section className="section-spacing bg-[var(--bg-secondary)]">
        <div className="container">
          <div className="max-w-3xl mx-auto flex flex-col md:flex-row justify-between gap-4">
            <Link href="/academy/ai-at-scale/module-5" className="flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"><ArrowLeftIcon /> Module 5: Governance Framework</Link>
            <Link href="/academy/ai-at-scale/module-7" className="btn btn-primary group inline-flex items-center gap-2">Next: Process Integration <ArrowRightIcon /></Link>
          </div>
        </div>
      </section>
    </div>
