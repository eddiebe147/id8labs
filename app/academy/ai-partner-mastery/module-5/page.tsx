      ...
      <section className="relative py-20 bg-zone-text">
        <div className="container">
          <m.div initial="initial" animate="animate" variants={stagger} className="max-w-3xl">
            ...
            <m.h1 variants={fadeUp} className="text-4xl md:text-5xl font-bold tracking-tight mb-6">The Editing Partner</m.h1>
            <m.p variants={fadeUp} className="text-xl text-[var(--text-secondary)]">Get AI to improve your writing without stealing your voice. The revision dance that preserves you.</m.p>
          </m.div>
        </div>
      </section>
      <section className="section-spacing">
        <div className="container"><div className="max-w-3xl mx-auto"><div className="p-8 bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl text-center"><p className="text-lg text-[var(--text-secondary)] mb-4">Full module content coming soon.</p><p className="text-sm text-[var(--text-tertiary)]">You'll build: Voice-preserving editing prompt</p></div></div></div>
      </section>
      <section className="section-spacing bg-[var(--bg-secondary)]">
        <div className="container">
          <div className="max-w-3xl mx-auto flex flex-col md:flex-row justify-between gap-4">
            <Link href="/academy/ai-partner-mastery/module-4" className="flex items-center gap-2 text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"><ArrowLeftIcon /> Module 4: Your Second Brain</Link>
            <Link href="/academy/ai-partner-mastery/module-6" className="btn btn-primary group inline-flex items-center gap-2">Next: Research Mode <ArrowRightIcon /></Link>
          </div>
        </div>
      </section>
    </div>
